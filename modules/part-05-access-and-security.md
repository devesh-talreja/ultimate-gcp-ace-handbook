...
> [!CAUTION]
> **Common Mistake:** Using the Compute Engine default service account (which historically has the broad roles/editor-equivalent scope) for production VMs is a textbook exam anti-pattern. Correct practice: create a purpose-built SA with only the roles that workload needs, and attach that instead.
